...
{
    return QTest::toPrettyCString(ba.constData(), 
ba.length());
}
...
template<> inline char *toString(const QChar &c)
{
    ...
    if (uc < 128) {
        char msg[32] = {'\0'};
        qsnprintf(msg, sizeof(msg), 
"QChar: '%c' (0x%x)", char(uc), unsigned(uc));
        return qstrdup(msg);
    }
...
template<> inline char *toString(const QModelIndex &idx)
{
    char msg[128];
    qsnprintf(msg, sizeof(msg), 
"QModelIndex(%d,%d,%p,%p)", idx.row(), idx.column(), idx.internalPointer(), idx.model());
    return qstrdup(msg);
}
#endif
...
inline char *toString(const QPair<T1, T2> &pair)
{
    ...
    return toString(QString::asprintf(
"QPair(%s,%s)", first.data(), second.data()));
}
template <typename T1, typename T2>
inline char *toString(const std::pair<T1, T2> &pair)
{
    ...
    const QScopedArrayPointer<char> second(toString(pair.second));
    return toString(QString::asprintf(
"std::pair(%s,%s)", first.data(), second.data()));
}
...
"Compared lists have different sizes.\n"
    ...
    for (int i = 0; isOk && i < actualSize; ++i) {
        if (!(t1.at(i) == t2.at(i))) {
            ...
            qsnprintf(msg, sizeof(msg), 
"Compared lists differ at index %d.\n"
                      "   Actual   (%s): %s\n"
    ...
    return compare_helper(isOk, msg, nullptr, 
