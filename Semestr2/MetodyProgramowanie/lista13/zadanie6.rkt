...
(define (extend-env [env : Env] [x : Symbol] [v : Exp]) : Env
  (cons (bind x (expS v)) env))


(define (extend-env-VAL [env : Env] [x : Symbol] [v : Value]) : Env
  (cons (bind x (valS v)) env))


(define (lookup-env [n : Symbol] [env : Env]) : Storable
...
(define (remove-from-env [n : Symbol] [env : Env]) : Env
  (type-case (Listof Binding) env
    [empty (error 'remove "unbound variable")]
    [(cons b rst-env) (cond
                        [(eq? n (bind-name b))
                          rst-env]
                        [else (cons b (remove-from-env n rst-env))])]))
...
(define (eval [e : Exp] [env : (Boxof Env)]) : Value 
  (type-case Exp e
    [(numE n) (numV n)]
    ...
    [(ifE b l r)
     (type-case Value (eval b env)
       ...
       [else
        (error 'eval "type error")])]
    [(varE x)
     (let ([wynik (lookup-env x (unbox env))])
       (type-case Storable wynik
         ([valS v] v)
         ([expS e]
            (let ([wartosc (eval e (box (remove-from-env x (unbox env))))])
              (begin
                (display (unbox env)) ;we only entry there once, so second x isnt calculated again
                (display "\n")
                (display "\n")
                (display "\n")
                (set-box! env (extend-env-VAL (unbox env) x wartosc))
                wartosc)))))]
    [(letE x e1 e2)
       (eval e2 (box (extend-env (unbox env) x e1)))]
    [(lamE x b)
     (funV x b (unbox env))]
    [(appE e1 e2)
     (let ([func (eval e1 env)])
          (type-case Value func
             [(funV x b env)
              (eval b (box (extend-env env x e2)))]
             [else (error 'apply "not a function")]))]))

(define (apply [v1 : Value] [v2 : Exp]) : Value
  (type-case Value v1
    [(funV x b env)
     (eval b (box (extend-env env x v2)))]
    [else (error 'apply "not a function")]))

(define (run [e : S-Exp]) : Value
  (eval (parse e) (box mt-env)))

;przyklad:
;(run `{if {= 1 0} {{lambda {x} {x x}} {lambda {x} {x x}}} 0}) ;if it wasnt lazy it would be looped permamently
;;;;;;;(run `{let x {+ 3 1} {+ x x}});;;;;
(module+ test
  (test (run `2)
        (numV 2))
...
(define (main [e : S-Exp]) : Void
  (print-value (eval (parse e) (box mt-env))))