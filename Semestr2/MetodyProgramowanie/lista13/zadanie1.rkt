...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{set-box! ANY ANY} s)
     (setBoxE (parse (second (s-exp->list s))) (parse (third (s-exp->list s))))]  
    ...
    [(setE x e0)
     (with [(v0 sto0) (eval e0 env sto)]
       (let ([sto1 (update-sto sto0 (lookup-env env x) (valS v0))])
         (v*s (voidV) sto1)))]
    [(boxE e0)
     (with [(v sto0) (eval e0 env sto)]
       (let ([l*s (alloc-sto sto0 (valS v))])
         (v*s (numV (fst l*s)) (snd l*s))))]
    [(unboxE e0)
     (with [(v0 sto0) (eval e0 env sto)]
       (let ([l (deref-sto sto0 (numV-n v0))])
         (v*s l sto0)))]
    [(setBoxE e1 e2)
     (with [(v1 sto1) (eval e1 env sto)]
       (with [(v2 sto2) (eval e2 env sto1)]
             (v*s (voidV) (update-sto sto2 (numV-n v1) (valS v2)))))]))
...
(run `{let x {box 3} {begin {set-box! x 4} {unbox x}}})
