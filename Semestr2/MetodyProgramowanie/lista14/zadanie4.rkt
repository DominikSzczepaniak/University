...
(define (showM [c : (M Value)]) : String
  (type-case (E 'a) (c 0)
    [(valE val)
     (type-case Value (fst val)
       [(numV val) (string-app "value: " (to-string val) ", state: " (to-string (snd (valE-val (c 0)))))]
       [(funV f) (string-app "value: " "#<procedure>" ", state: " (to-string (snd (valE-val (c 0)))))])]
    [(errE l m) (string-append "error in " (string-append (to-string l) (string-append ": " m)))]))
...
(define (lookup-env [x : Symbol] [env : Env]) : (M Value)
  (type-case (Listof Binding) env
    [empty
     (λ (s) (errE 'lookup-env "unbound variable"))]
    [(cons b rst-env)
     (cond
       [(eq? x (bind-name b))
        (returnM (bind-val b))]
