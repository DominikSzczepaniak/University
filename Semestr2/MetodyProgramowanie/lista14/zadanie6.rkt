...
(define (bindHelp [c : (M 'a)] [f : ('a -> (M 'b))] [answer : (M 'b)])
  (if (empty? c)
      answer
      (bindHelp (rest c) f (append answer (f (first c))))))
...
(define (showHelper [c : (M Value)] answer) : String
  (if (empty? c)
      answer
      (type-case Value (first c)
        [(numV n) (showHelper (rest c) (string-append answer (string-append (to-string (numV-n (first c))) " ; ")))]
        [(funV f) (showHelper (rest c) (string-append answer (string-append "#<procedure>" " ; ")))])))
...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{amb ANY ANY} s)
     (ambE (parse (second (s-exp->list s))) (parse (third (s-exp->list s))))]
    [(s-exp-match? `fail s)
     (failE)]
