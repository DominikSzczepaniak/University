...
(define (showM [c : (M Value)]) : String
  (string-app "value: " (to-string (fst c)) ", output: " (to-string (snd c))))
    ...
    [(appE e0 e1)
     (bindM (eval e0 env)
            (λ (v0) (bindM (eval e1 env)
                           (λ (v1) (apply v0 v1)))))]
    [(writeE e)
     (pair (voidV) (append (snd (eval e env)) (list (numV-n (fst (eval e env))))))]
    [(beginE e1 e2)
     (let ([lista (snd (eval e1 env))])
     (if (writeE? e2)
         (pair (voidV) (append lista (snd (eval e2 env))))
         (pair (fst (eval e2 env)) (append lista (snd (eval e2 env))))))]))
