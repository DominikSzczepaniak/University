...
(define (showM [c : (M Value)]) : String
  (type-case (M Value) c
    ...
    [(errM l m) (string-append (string-append (string-append "error in " (to-string l)) ": ") m)]))
...
(define (lookup-env [x : Symbol] [env : Env]) : (M Value)
  (type-case (Listof Binding) env
    [empty
     (errM 'lookup-env "unbound variable")]
    [(cons b rst-env)
     (cond
       [(eq? x (bind-name b))
        (returnM (bind-val b))]
...
(define (apply [v0 : Value] [v1 : Value]) : (M Value)
  (type-case Value v0
    [(funV f) (f v1)]
    [else (errM 'apply "not a function")]))
