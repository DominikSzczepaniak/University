...
(define-syntax string-app
  (syntax-rules ()
    [(string-app a)
     (string-append a "")]
    [(string-app a1 a2 a3 ...)
     (string-append a1 (string-append a2 (string-append a3 ...)))]))
...
(define (bindM [c : (M 'a)] [f : ('a -> (M 'b))]) : (M 'b)
  (λ (s)
    (let ([result (c s)])
      ((f (valE-val (fst result))) (snd result)))))

(define (errorM [l : Symbol] [m : String]) : (M 'a)
  (error l m))

(define (showM [c : (M Value)]) : String
  (type-case (E 'a) (fst (c 0))
    [(valE val)
     (type-case Value val
       [(numV val) (string-app "value: " (to-string val) ", state: " (to-string (snd (c 0))))]
       [(funV f) (string-app "value: " "#<procedure>" ", state: " (to-string (snd (c 0))))])]
    [(errE l m) (string-append "error in " (string-append (to-string l) (string-append ": "
                                                                                       (string-append m (string-append ", state: "
                                                                                                                        (to-string (snd (c 0))))))))]))
...
(define (lookup-env [x : Symbol] [env : Env]) : (M Value)
  (type-case (Listof Binding) env
    [empty
     (λ (s) (pair (errE 'lookup-env "unbound variable") s))]
    [(cons b rst-env)
     (cond
       [(eq? x (bind-name b))
        (returnM (bind-val b))]
    ...
    (type-case Value v1
      [(numV n1)
       (type-case Value v2
         ...
         [else
          (λ (s) (pair (errE 'prim-op "not a number") s))])]
      [else
       (λ (s) (pair (errE 'prim-op "not a number") s))])))
...
(module+ test
  ...
  (test/exn (run `x)
            "unbound variable")
  (test (run `{+ {+ count count} {+ count count}})
        "value: 2, state: 3"))
