    ...
    [(letrecE x e1 e2) 
     (let* ([new-env (extend-env-undef env x)]
       ...
       (begin (update-env! new-env x v1) 
              (eval e2 new-env)))]
    #;[(letrecE bindings e) ;if i wanted to extend letrec
     (let* ([new-env (extend-env-undef env (map car bindings))]
            [values (map (λ (b) (eval (cadr b) new-env)) bindings)])
       (begin
         (for-each (λ (b v) (update-env! new-env (car b) v)) bindings values)
         (eval e new-env)))]
