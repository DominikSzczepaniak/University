...
(define (valuetoexp v)
  (type-case Value v
    [(numV n) (numE n)]
    [(boolV b) (boolE b)]
    [(funV x e env) (lamE x e)]
    [(primopV f) (numE 1)]
    [(recV f x e env) (recE f x e)]))
...
(define (apply [v1 : Value] [v2 : Value]) : Value
  (type-case Value v1
    ...
    [(primopV f)
     (f v2)]
    [(recV f x e env) ;chcemy sobie zmienic e na lambde ktora przyjmuje x zeby to wykonac i rozszerzyc srodowisko
     ;o to ze f to v1
     ; czyli chcemy zwrocic zaaplikowane v2 (jako x) w e z rozszerzonym srodowiskiem env o to ze f to v1
     (eval (appE (lamE x e) (valuetoexp v2)) (extend-env env f v1))]
...
(define (value->string [v : Value]) : String
  (type-case Value v
    ...
    [(primopV f) "#<primop>"]
    [(recV x a b c) "#<recursive_procedure>"]))
...
(run `{{rec func {n} {if {= n 0} 1 {* n {func {- n 1}}}}} 5}) 

;Konstrukcja letrec w ogólnej postaci pozwala na definiowanie wielu wzajemnie rekurencyjnych funkcji.
;Oznacza to, że funkcje wzajemnie odwołują się do siebie nawzajem, co sprawia, że ich
;definicje muszą być dostępne wewnątrz swojego zakresu. Jest to inaczej niż w przypadku pojedynczej
;rekurencyjnej funkcji, gdzie funkcja jest widoczna tylko wewnątrz swojego własnego ciała.