...
(define (applyA e1 e2)
  (type-case ValueA e1
    ...
    [(boolVA n) (boolVA n)]
    [(funVA e1 env)
     (let ([new-env (extend-envA env e2)])
       (evalA e1 new-env))]))
...
;Dzięki takiej reprezentacji możemy śledzić zasięg zmiennych i odnosić się do odpowiednich zmiennych w domknięciach funkcji. Gdybyśmy używali nazw zamiast indeksów, mogłoby to prowadzić do konfliktów nazw, zwłaszcza w przypadku zagnieżdżonych funkcji i różnych poziomów zasięgu.

;Reprezentacja z indeksami dobrze współgra z domknięciami funkcji, ponieważ pozwala nam
;uniknąć tych konfliktów i jednoznacznie określa, które zmienne są związane w jakim zasięgu.
