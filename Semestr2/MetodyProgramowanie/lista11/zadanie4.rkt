...
;; translating back functions


(define (fresh-names [env : (EnvA 'a)] i answers)
  (if (empty? env)
      answers
      (fresh-names (rest env) (+ i 1) (string->symbol (string-append "x" (to-string i))))))


(define (fresh-name i) : Symbol
   (string->symbol (string-append "x" (to-string i))))

(define (translate_back [e : ExpA] [env : Env]) : Exp
  (type-case ExpA e
    [(numA n)
     (numE n)]
    [(opA o l r)
     (opE o (translate_back l env) (translate_back r env))]
    [(ifA b l r)
     (ifE (translate_back b env)
          (translate_back l env)
          (translate_back r env))]
    [(varA n) 
     (varE (fresh-name (- (length env) n)))] ;we have it reversed
    [(letA a1 a2)
     (letE (fresh-name (+ 1 (length env)))
           (translate_back a1 env)
           (translate_back a2 (extend-env env (fresh-name (+ 1 (length env))) (eval (translate_back a1 env) env))))]))


(define (runTB [s : S-Exp])
  (eval (translate_back (translate (parse s) mt-envA) mt-env) mt-env))