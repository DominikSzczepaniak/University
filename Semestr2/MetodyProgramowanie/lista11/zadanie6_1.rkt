...
(define (parse-lambda-list s wyr)
  (if (empty? s)
      wyr
      (lamE (s-exp->symbol (first s)) (parse-lambda-list (rest s) wyr))))

(define (parse-appE-list s lam)
  (if (empty? s)
      lam
      (parse-appE-list (rest s) (appE lam (parse (first s))))))
...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{lambda ANY ANY} s) 
     (parse-lambda-list (s-exp->list (second (s-exp->list s))) (parse (third (s-exp->list s))))]
    ...
    [(s-exp-match? `{ANY ANY} s)
     (let ((first-exp (parse (first (s-exp->list s)))))
       (if (lamE? first-exp)
           (parse-appE-list (s-exp->list (second (s-exp->list s))) first-exp)
           (appE first-exp
                 (parse (second (s-exp->list s))))))]
