...
(define (fresh-var [n : Number]) : Symbol
  (string->symbol (string-append "var_" (to-string n))))


(define (znajdz [a : Symbol] [b : (Listof (Symbol * Symbol))]) : Symbol
  (if (empty? b)
      a
      (if (eq? (fst (first b)) a)
          (snd (first b))
...
(define (w-liscie [a : Symbol] [b : (Listof (Symbol * Symbol))]) : Boolean
  (if (empty? b)
      #f
      (if (eq? (fst (first b)) a)
          #t
...
;(parse `{let x 1 {let x 2 {let x 3 {let y 1 y}}}})
;(parse `{let v1 1 {let v1 2 {let v1 3 {let v2 1 v2}}}

;; precondition: a is a closed expression
(define (subst-closed [e : Exp] [n : Number] [listChange : (Listof (Symbol * Symbol))]) : Exp
  (type-case Exp e
    [(numE n)
     (numE n)]
    [(trueE)
    ...
    [(falseE)
     (falseE)]
    [(opE o l r)
     (opE o (subst-closed l n listChange) (subst-closed r n listChange))]
    [(ifE b l r)
     (ifE (subst-closed b n listChange) (subst-closed l n listChange) (subst-closed r n listChange))]
    [(varE y) 
     (varE (znajdz y listChange))]
    [(letE y e1 e2)
      (if (w-liscie y listChange)
          (let ([zmiana (znajdz y listChange)])
            (letE zmiana (subst-closed e1 n listChange) (subst-closed e2 n listChange)))
          (let ([new-y (fresh-var n)])
            (letE new-y (subst-closed e1 n listChange) (subst-closed e2 (+ 1 n) (append listChange (list (pair y new-y)))))))]))
