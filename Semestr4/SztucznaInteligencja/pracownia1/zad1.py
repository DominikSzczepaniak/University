...
def viable_move(white_king, white_rook, black_king, id):
    """Checks if move is possible
    Input: new positions of pieces and id of pieces which moved
    1: white_king
    2: white_rook
    3: black_king"""
    if id == 1:
        ...
        if white_king == white_rook or white_king == black_king:
            return False
        for direction in directions:
            new_position = (white_king[0] + direction[0], white_king[1] + direction[1])
    ...
    if id == 2:
        ...
        if white_rook == white_king or white_rook == black_king:
            return False
        for direction in directions:
            new_position = (white_rook[0] + direction[0], white_rook[1] + direction[1])
            if not inside_board(new_position):
                continue
            if new_position == black_king:
                return False
    ...
    if id == 3:
        ...
        if black_king[0] == white_rook[0] or black_king[1] == white_rook[1]:
            return False
        for direction in directions:
            new_position = (black_king[0] + direction[0], black_king[1] + direction[1])
            if not inside_board(new_position):
                continue
...
def possible_move(white_king, white_rook, black_king):
    """Checks if there is any possible move of black_king"""
    for direction in directions:
        new_position = (black_king[0] + direction[0], black_king[1] + direction[1])
        possible = True
        if not inside_board(new_position):
            continue
        # check if rook is in the same row or column
        if new_position[0] == white_rook[0] or new_position[1] == white_rook[1]:
            possible = False
        # check if king is in the range of move
        for direction2 in directions:
            if new_position[0] + direction2[0] == white_king[0] and new_position[1] + direction2[1] == white_king[1]:
                possible = False
        if possible:
            return True
    return False
...
def check_draw(color, white_king, white_rook, black_king):
    if (color == "black" and not possible_move(white_king, white_rook, black_king) and (
...
if __name__ == "__main__":
    ...
    for line in file:
        ...
        if not check_input(white_king, white_rook, black_king):
            print("INF")
            continue
        # score, moves = debug_mode(start_color, white_king, white_rook, black_king)
        score = evaluate_mode(start_color, white_king, white_rook, black_king)
