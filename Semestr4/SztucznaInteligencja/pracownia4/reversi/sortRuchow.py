        ...
        def alphabeta(state, depth, alpha, beta, maximizing_player, ile):
            ...
            if maximizing_player:
                ...
                for move in children:
                    ...
                    moves.append((new_board.heuristic(ile+1), new_board))
                #sort malejacy po moves[0]
                sorted_moves = sorted(moves, key=lambda x: x[0], reverse=True)
                for move in sorted_moves:
                    value = max(value, alphabeta(move[1], depth - 1, alpha, beta, False,  ile + 1))
                ...
                for move in children:
                    ...
                    moves.append((new_board.heuristic(ile+1), new_board))
                sorted_moves = sorted(moves, key=lambda x: x[0])
                for move in sorted_moves:
                    value = min(value, alphabeta(move[1], depth - 1, alpha, beta, True, ile + 1))
                    beta = min(beta, value)
                    if beta <= alpha:
                        break
...
for z in range(1, nr):
    ...
    if score >= 0:
        sumGames += 1
    print('Game number: ', z, ", won till now: ", sumGames, ", lost till now: ", z - sumGames, " score in this game: ", score,
